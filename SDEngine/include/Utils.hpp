...
inline void SingleTimeCommand(const vk::Device& device, const vk::Queue& queue, const vk::CommandPool& commandPool, const std::function<void(const vk::CommandBuffer&)>& action) {
...
inline uint32_t FindMemoryType(const vk::PhysicalDevice& physicalDevice, uint32_t typeFilter, vk::MemoryPropertyFlags properties) {
...
inline void CopyBufferToImage(const vk::CommandBuffer& cmdBuffer, const vk::Buffer& buffer, const vk::Image& image, uint32_t width, uint32_t height) {
...
inline std::expected<Texture, std::string> CreateTexture(
    const vk::Device& device,
    const vk::PhysicalDevice& physicalDevice,
    ...
    const std::filesystem::path& filePath)
{
    ...
    auto [image, imageMemory] = CreateImage(
        device, physicalDevice, texWidth, texHeight, vk::Format::eR8G8B8A8Srgb, vk::ImageTiling::eOptimal,
        vk::ImageUsageFlagBits::eTransferDst | vk::ImageUsageFlagBits::eSampled, vk::MemoryPropertyFlagBits::eDeviceLocal);

    SingleTimeCommand(device, graphicsQueue, commandPool, [&](const vk::CommandBuffer& cmdBuffer) {
        TransitionImageLayout(cmdBuffer, *image, vk::Format::eR8G8B8A8Srgb, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal);
        CopyBufferToImage(cmdBuffer, *stagingBuffer, *image, static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
        TransitionImageLayout(cmdBuffer, *image, vk::Format::eR8G8B8A8Srgb, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eShaderReadOnlyOptimal);
    });
...
inline vk::UniqueShaderModule CreateShaderModule(const vk::Device& device, const std::vector<char>& code) {
