...
auto EntityManager::View() {
  if constexpr (sizeof...(Args) == 1) {
    ...
    // Check if it's a group we can unpack
    if constexpr (requires { typename UnpackGroup<T>::type; }) {
      return typename UnpackGroup<T>::type(*this);
    } else {
      // Single argument, but not a group (e.g. View<Transform>())
      return ViewImpl<Args...>(*this);
    }
  } else {
    // Multiple arguments (e.g. View<Transform, Mesh>())
    return ViewImpl<Args...>(*this);
  }
}
template<typename T>
